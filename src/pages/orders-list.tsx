...
const OrdersList = (props: any) => {
  ...
  useEffect(() => {
    setOrders(() => {
      let orders = state.orders.filter((o: any) => (props.type === 's' && o.status === props.id) || (props.type === 'u' && o.userId === props.id))
      orders = orders.map((o: any) => {
        const userInfo = state.users.find((u: any) => u.id === o.userId)
  ...
  return(
    <Page>
      <Navbar title={`${labels.orders} ${props.type === 's' ? orderStatus.find(s => s.id === props.id)?.name : state.customers.find((c: any) => c.id === props.id).name}`} backLink={labels.back}>
      <NavRight>
