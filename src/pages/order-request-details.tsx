...
const OrderRequestDetails = (props: any) => {
  ...
  const [orderBasket] = useState(() => {
    ...
    order.requestBasket.forEach((p: any) => {
      const index = basket.findIndex((bp: any) => bp.packId === p.packId)
      if (index === -1) {
        basket.push({
          ...p,
          change: p.quantity
  ...
  const [total] = useState(() => orderBasket.reduce((sum: any, p: any) => sum + p.price * p.quantity, 0))
