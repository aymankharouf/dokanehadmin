...
const PackImage = props => {
  ...
  return (
    <div className="relative">
      <img src={product.imageUrl} className="img-list" alt={product.name} />
      {props.pack.offerQuantity > 1 ? 
        <span className="offer-quantity-list">{`× ${props.pack.offerQuantity}`}</span> 
      : ''}
      {props.pack.bonusPackId ? 
        <div>
          <img src={bonusProduct.imageUrl} className="bonus-img-list" alt={bonusProduct.name} />
          {props.pack.bonusQuantity > 1 ? 
            <span className="bonus-quantity-list">{`× ${props.pack.bonusQuantity}`}</span> 
          : ''}
        </div>
      : ''}
    </div>
