...
const RequestedPackDetails = (props: any) => {
	const { state, dispatch } = useContext(StoreContext)
  const [error, setError] = useState('')
  const [pack] = useState(() => state.packs.find((p: any) => p.id === props.packId))
  const [basketStockQuantity, setBasketStockQuantity] = useState<any>('')
  const [packStores, setPackStores] = useState([])
  useEffect(() => {
    setBasketStockQuantity(() => {
      const basketStock = state.basket.storeId === 's' && state.basket.packs.find((p: any) => p.packId === props.packId || state.packs.find((pa: any) => pa.id === p.packId && (pa.subPackId === props.packId || pa.bonusPackId === props.packId)))
  ...
  const addToBasket = (packStore: any, exceedPriceType: any) => {
    try {
      let quantity, params
      if (packStore.packInfo.byWeight) {
        ...
        const mainPackInfo = state.packs.find((p: any) => p.subPackId === packStore.packId && !p.forSale)
        const mainPackStore = state.packPrices.find((p: any) => p.storeId === packStore.storeId && p.packId === mainPackInfo.id)
	...
	const handlePurchase = (packStore: any) => {
    try{
      if (state.basket.storeId && state.basket.storeId !== packStore.storeId){
        throw new Error('twoDiffStores')
      }
      const packInfo = state.packs.find((p: any) => p.id === packStore.packId)
      if (packInfo.byWeight){
        if (state.basket.packs?.find((p: any) => p.packId === packInfo.id && p.orderId === props.orderId)) {
          throw new Error('alreadyInBasket')
        }
      } else {
        if (state.basket.packs?.find((p: any) => p.packId === packInfo.id)) {
          throw new Error('alreadyInBasket')
        }
      }
