...
const EditSpending = (props: any) => {
  const { state } = useContext(StoreContext)
  const [error, setError] = useState('')
  const [spending] = useState(() => state.spendings.find((s: any) => s.id === props.id))
  const [type, setType] = useState(spending.type)
  const [amount, setAmount] = useState<any>((spending.amount / 100).toFixed(2))
  const [spendingDate, setSpendingDate] = useState<any>(() => spending.spendingDate ? [spending.spendingDate.toDate()] : '')
