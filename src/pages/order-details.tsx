...
const OrderDetails = (props: any) => {
  ...
  useEffect(() => {
    setOrder(() => state.orders.find((o: any) => o.id === props.id))
  }, [state.orders, props.id])
  useEffect(() => {
    setOrderBasket(() => order.basket.map((p: any) => {
      const storeName = p.storeId ? (p.storeId === 'm' ? labels.multipleStores : state.stores.find((s: any) => s.id === p.storeId).name) : ''
  ...
  useEffect(() => {
    setLastOrder(() => {
      const userOrders = state.orders.filter((o: any) => o.id !== order.id && o.userId === order.userId && !['c', 'm', 'r'].includes(o.status))
      userOrders.sort((o1: any, o2: any) => o2.time.seconds - o1.time.seconds)
  ...
  const handleAction = (action: any) => {
    try{
      if (action.path) {
        ...
        if (action.id === 'a' && !state.customers.find((c: any) => c.id === order.userId)){
          ...
          f7.dialog.confirm(labels.confirmMergeText, labels.confirmationTitle, () => {
            try{
              ...
              for (let p of order.basket) {
                found = lastOrder.basket.find((bp: any) => bp.packId === p.packId)
                if (found && found.price !== p.price) {
                  throw new Error('samePackWithDiffPrice')
                }
                if (found && found.weight > 0 && state.packs.find((pa: any) => pa.id === p.packId).isDivided) {
  ...
  return(
    <Page>
      ...
      <Actions opened={actionOpened} onActionsClosed={() => setActionOpened(false)}>
        ...
        {props.type === 'n' && statusActions.map((a: any) => 
