...
const ConfirmPurchase = () => {
  ...
  const [basket] = useState(() => state.basket.packs.map((p: any) => {
  ...
  const [total] = useState(() => state.basket.packs.reduce((sum: any, p: any) => sum + Math.round(p.cost * (p.weight || p.quantity)), 0))
  useEffect(() => {
    if (error) {
      showError(error)
      setError('')
