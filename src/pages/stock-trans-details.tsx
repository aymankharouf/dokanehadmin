...
const StockTransDetails = (props: any) => {
  const { state } = useContext(StoreContext)
  const [stockTrans] = useState(() => props.type === 'a' ? state.archivedStockTrans.find((t: any) => t.id === props.id) : state.stockTrans.find((t: any) => t.id === props.id))
  const [stockTransBasket, setStockTransBasket] = useState([])
  useEffect(() => {
    setStockTransBasket(() => stockTrans.basket.map((p: any) => {
  ...
  return(
    <Page>
      <Navbar title={`${stockTransTypes.find(ty => ty.id === stockTrans.type)?.name} ${stockTrans.storeId ? state.stores.find((s: any) => s.id === stockTrans.storeId).name : ''}`} backLink={labels.back} />
      <Block>
        <List mediaList>
          {stockTransBasket.map((p: any) => 
            <ListItem 
